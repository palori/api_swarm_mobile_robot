...
void COMM_TSY::write_serial(bool newCommand, double _odo[3], float _ir[2], float battery ,int _imu_cmps[3], int _imu_gyro[3], int _imu_accel[3], bool _obstacle_found){ //params might change, maybe add last sensors data
	// modify to add this
...
String COMM_TSY::sensorData2msg(bool newCommand, double _odo[3], float _ir[2], float battery,int _imu_cmps[3], int _imu_gyro[3], int _imu_accel[3], bool _obstacle_found){
	Command command;
	...
	*/
	...
	new_msg += command.NC+"=" + String(newCommand);
