...
float R_WEEL = 0.045; // [m]

float speed = -1.0;
int pulse[2] = {0, 0};
int newpulse[2] = {1, 1};
float velocity = 0;
float K_amplifier = 4.0;
...
void loop()
{
  ...
  if (Serial.available()) {
    ...
    {
      enableMotors();
      Setpoint[0] = (double)speed * K_amplifier;
      Setpoint[1] = (double)speed * K_amplifier;
  ...
  //pid1.Compute();
  //pid2.Compute();
  Output[0] = Setpoint[0];//-Velocity[0]*K_amplifier;
  Output[1] = Setpoint[1];//-Velocity[1]*K_amplifier;
  ...
  if (speed > -1) {
    ...
    s += "," + String(speed) + "," + String(speed); //s += ","+String(Setpoint[0])+","+String(Setpoint[1]);
