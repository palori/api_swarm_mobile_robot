...
void processAccelData(){
  ...
  if (gForceX > 2.0) gForceX -= 4.0;
  if (gForceY > 2.0) gForceY -= 4.0;
  if (gForceZ > 2.0) gForceZ -= 4.0;
...
void recordTempRegisters() {
  ...
  Wire.endTransmission();
  Wire.requestFrom(0b1101000,2); //Request Gyro Registers (41 - 42)
  while(Wire.available() < 2);
  temp = Wire.read()<<8|Wire.read(); //Store first two bytes into 
  processTempData();
}

void processTempData() {
  temperature = temp / 340.0 - 175.0; // + 35.0;
}

void getAngles(){
  roll = atan2(gyroZ,gyroY) * 180.0 / 3.141592;
  pitch = atan2(gyroZ,gyroX) * 180.0 / 3.141592;
}
